...
def INSTAGRAN(file_name, DELAY, choose):
    ...
    for idx, line in enumerate(lines, 1):
        ...
        print(f"\n\033[1;33m=== Bắt đầu xử lý cookie với: {NAMEIG} (dòng {idx}) ===\033[0m")
        ...
        try:
            thongtinig = requests.get('https://gateway.golike.net/api/instagram-account', headers=headersgl).json()
        ...
        while dem < choose and DIE == 0:
            try:
                ...
                getjop = requests.get('https://gateway.golike.net/api/advertising/publishers/instagram/jobs', params=paramsgetjop, headers=headersgl).json()
                if getjop['status'] == 200:
                    ...
                    if job_type == 'follow':
                        ...
                        if '"status":"ok"' in response:
                            ...
                            time.sleep(3)
                            response = requests.post(url, headers=headersgl, json=json_data).json()
