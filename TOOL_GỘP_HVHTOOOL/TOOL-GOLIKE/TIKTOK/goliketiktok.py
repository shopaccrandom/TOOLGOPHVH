...
def chonacc():
  ...
  response = requests.get('https://gateway.golike.net/api/tiktok-account', headers=headers, json=json_data,impersonate="chrome").json()
  return response
def nhannv(account_id):
  ...
  response = requests.get('https://gateway.golike.net/api/advertising/publishers/tiktok/jobs',params=params,headers=headers,json=json_data,impersonate="chrome").json()
  return response
...
def baoloi(ads_id,object_id,account_id,loai):
  ...
  response = requests.post('https://gateway.golike.net/api/report/send', headers=headers, json=json_data1,impersonate="chrome").json()
...
while True:
  ...
  if(nhanjob["status"] == 200):
    ...
    if "com.termux" in os.environ.get("PREFIX", ""):
        ...
        os.system(f'start "" "{link}"')  # cần "" để tránh lỗi khi đường dẫn chứa khoảng trắng
    else:
