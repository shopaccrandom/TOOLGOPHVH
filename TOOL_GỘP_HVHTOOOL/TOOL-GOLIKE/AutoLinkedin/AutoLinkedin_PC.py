# nếu bị lỗi thư viện này --> curl_cffi thì có thể python không hỗ trợ hoặc lỗi gì đó , hãy xóa python đi là cài lại phiên bản mới nhất
try :
...
def LINKEDIN():
    checkaccount = requests.get('https://gateway.golike.net/api/linkedin-account', headers=headers, impersonate="chrome").json()
    ...
    if choose >=1 or choose <= len(user_linkedin1) :
        ...
        for i in range(choose):
                ...
                checkurl2 = ses.get(url2,headers=headers, impersonate="chrome").json()
